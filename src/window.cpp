...
const QString Window::WIDTH_KEY = "width";
const QString Window::HEIGHT_KEY = "height";
...

{
	...
	qint32 w = settings.value(WIDTH_KEY, 1024).toInt();
	qint32 h = settings.value(HEIGHT_KEY, 768).toInt();
    resize(w, h);
}
...
void Window::resizeEvent(QResizeEvent* event)
{
	...
	QSettings settings;
	settings.setValue(WIDTH_KEY, width());
	settings.setValue(HEIGHT_KEY, height());
}

void Window::on_open()
{
    QString filename = QFileDialog::getOpenFileName(
                this, "Load .drc file", QString(), "Draco files (*.drc, *.DRC)");
    if (!filename.isNull())
    {
        load_drc(filename);
    }
...
void Window::on_about()
{
    QMessageBox::about(this, "",
        "<p align=\"center\"><b>fdrc</b><br>" FDRC_VERSION "</p>"
        "<p>A fast viewer for <code>.drc</code> files.<br>"
        "<a href=\"https://github.com/Maeyanie/fdrc\""
        "   style=\"color: #93a1a1;\">https://github.com/Maeyanie/fdrc</a></p>"
        "<p>© 2025 Maeyanie<br>"
...
void Window::on_bad_stl()
{
    QMessageBox::critical(this, "Error",
                          "<b>Error:</b><br>"
                          "This <code>.drc</code> file is invalid or corrupted.<br>"
...
bool Window::load_drc(const QString& filename, bool is_reload)
{
...
void Window::dragEnterEvent(QDragEnterEvent *event)
{
    if (event->mimeData()->hasUrls())
    {
        ...
        if (urls.size() == 1 && urls.front().path().endsWith(".drc"))
...
{
    load_drc(event->mimeData()->urls().front().toLocalFile());
}
...
void Window::build_folder_file_list()
{
    ...
    QDirIterator dirIterator(lookup_folder, QStringList() << "*.drc", QDir::Files | QDir::Readable | QDir::Hidden);
...
bool Window::load_prev(void)
{
    ...
    return load_drc(neighbors.first);
}
...
bool Window::load_next(void)
{
    ...
    return load_drc(neighbors.second);
}
